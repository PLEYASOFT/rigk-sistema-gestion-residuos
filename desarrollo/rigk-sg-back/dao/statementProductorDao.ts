...
class statementProductorDao {
    ...
    public async getDeclaretionByYear(business: string, year: string) {
        ...
        res_header = await conn?.execute("SELECT * FROM header_statement_form WHERE ID_BUSINESS = ? AND YEAR_STATEMENT = ? ORDER BY ID DESC",[business, year]).then((res) => res[0]).catch(error => {undefined});
