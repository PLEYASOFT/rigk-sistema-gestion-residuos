...
export class MaintainerGoalsComponent implements OnInit {
    ...
    constructor(private fb: FormBuilder,
      public goalsTsService: GoalsTsService) { }
    ...
    saveForm() {
      ...
      if(this.isEditing) {
        this.goalsTsService.updateRatesByYear(this.ratesForm.value).subscribe({
          next: r=> {
            if(r.status){
              this.getAllGoals();
              console.log("ifff");
        ...
        this.goalsTsService.save(this.ratesForm.value).subscribe({
          next: r=> {
            if(r.status){
              this.getAllGoals();
              document.getElementById('closeModallll')?.click();
    ...
    btnUpdateBusiness(id_business: any) {
      ...
      this.goalsTsService.updateRates(this.id, this.price).subscribe({
        next: resp => {
          if (resp.status) {
            Swal.fire({
